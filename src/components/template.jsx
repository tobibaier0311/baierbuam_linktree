import { Avatar, Button, Chip, createTheme, Divider, Stack  } from "@mui/material";
...
export function Template() {
    return(
        ...
        {links.map((item) => (
            <Button size="large" sx={{ textTransform: "none", color: 'black'}} variant="outlined" className="chip" onClick={() => openLink(item.url)} startIcon={item.icon}>{item.name}</Button>
        ))}</Stack>
        </div>
    )
}